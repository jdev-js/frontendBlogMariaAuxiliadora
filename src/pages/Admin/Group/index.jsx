...
import WarnigGroup from '../../../components/WarnigGroup'
...
export default function GroupAdmin() {
  ...
  const { data, loading, refetch } = useQuery(GET_POSTS, { variables: { typePost: TYPE_POST.GROUP } })
  return (
    <div className={styles.containerPage}>
      <div className={styles.containerButton}>
        <ButtonLarge onClick={() => setShowModal(true)}>Nuevo Grupo</ButtonLarge>
      </div> 
...

      {
        showModal && (
          <Modal title={"Crear Post"} onClose={() => setShowModal(false)}>
            <FormCreatePost onFinally={() => setShowModal(false)} refetch={refetch} typePost={TYPE_POST.GROUP}/>
          </Modal>
        )
      }
    </div>
