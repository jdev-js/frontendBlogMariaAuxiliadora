      ...
      <div className='slide-container'>
        {
          images.map((image,index) => {
            return (
              <div
              className={index === currentSlide ? "slide active" : index === currentSlide - 1 || (index === 0 && currentSlide === images.length -1) ? "slide prev" : "slide next" } 
              key={index}>
                {index === currentSlide && ( <img src={image} alt={`Slide ${index}`}/> )}
              </div>
            ) 
          })
        }
        <div className='info-carrucel'>
          ...
          <h2>Ayudanos a manterner fundaciones. Dios te bendiga hoy siempre</h2>
